/**
 * SEA2 插件：ql-notify —— 接收青龙任务消息并【私发管理员】
 *
 ...
 *
 * 安全铁律：
 *   - 本插件只新增「私发管理员」能力，绝不调用 send_group_msg，绝不改动其它插件/框架代码。
...
class QlNotifyPlugin extends BasePlugin {
    ...
    async _broadcast(title, content, level) {
        ...
        if (!this.superAdmin) {
            this._log('warn: 未配置 superAdmin，消息被丢弃: %s', title);
            return [{ admin: null, ok: false }];
        }
        const ok = await this._sendPrivate(this.superAdmin, text);
        return [{ admin: this.superAdmin, ok }];
    }
    ...
    _handleWebhook(req, res) {
        ...
        req.on('end', async () => {
            ...
            try {
                ...
                res.statusCode = 200;
                res.end(JSON.stringify({ ok: true, sent: r, target: 'private:' + this.superAdmin }));
            } catch (e) {
                res.statusCode = 500;
